...
function closeEdit() { isEditOpen.value = false; selectedExpense.value = null }
async function saveEdit(updated) {
  const { data } = await updateExpense(props.userId, updated.id, { item: updated.item, cost: updated.cost, date: updated.date })
  if (data) {
...
<template>
  <div class="pb-16">
    ...
    <div class="space-y-2">
      ...
      <template v-else>
        <div v-for="(e, i) in expenses" :key="e.id ?? i" class="flex items-center justify-between  bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg p-3" @click="openEdit(e)">
    ...
    <EditExpenseModal v-if="selectedExpense" :isOpen="isEditOpen" :expense="selectedExpense" @close="closeEdit" @save="saveEdit" @delete="confirmAndDelete" />
  </div>
</template>